...
| `/wiki-ontology-apply [--plan 파일]`           | 온톨로지 도입/변경 시 기존 페이지 다수를 계획 파일 기반으로 일괄 태깅 |

슬래시 명령어 외에 자연어로도 동작합니다:

- _"raw/my-article.md 수집해줘"_
- _"query: 주요 테마가 뭐야?"_
- _"위키 검사해줘"_
- _"지식 그래프 만들어줘"_
...
> 세션 내에 쿼리가 없으면: _"No query found in this session. Run `/wiki-query <question>` first."_
...
`wiki/ontology.yaml`이 있을 경우 `python scripts/ontology_validate.py`가 자동 실행되어 온톨로지 관련 추가 검사(체크 7~12번: unknown class/predicate/phase, domain/range 위반, 필수 속성 누락, workflow 공백)를 수행하고 그 결과를 `## Ontology Violations` 섹션으로 `wiki/lint-report.md`에 병합합니다.
...
온톨로지가 없을 때는 `type:` 기준으로 색이 정해집니다:
...
`wiki/ontology.yaml`이 있으면 별도 설정 없이 그래프가 다음과 같이 바뀝니다:

- **축(axis) 색상**: `class:` 프런트매터가 있는 페이지는 `type:` 대신 소속 축(Actor/Activity/Artifact/Objective/Resource/Context) 색으로 표시됩니다.
- **Typed edge**: 각 페이지의 `relations:` 프런트매터(predicate/target)가 점선 엣지로 그려지며, 위키링크 엣지와 별도로 공존합니다.
- **구조적 허브 강조**: 연결 차수가 평균 + 2표준편차를 넘는 노드는 별 모양으로 크게 표시됩니다 (특정 class를 하드코딩하지 않고 실제 연결 구조 기준).
- **레거시 디밍 토글**: `status: legacy|archived|deprecated`로 태깅된 페이지는 화면에서 흐리게 표시되며, 좌측 패널의 "Dim legacy pages" 체크박스로 켜고 끌 수 있습니다. 온톨로지 없이도 `status:` 필드만 있으면 동작하는 범용 기능입니다.
...
| `/wiki-ontology-validate`            | 위키 페이지와 온톨로지 스키마 정합성 검사 (`scripts/ontology_validate.py`)    |
| `/wiki-ontology-apply [--plan 파일]` | 기존 페이지 다수를 계획 파일 기반으로 일괄 태깅 (`scripts/ontology_apply.py`) |

온톨로지를 정의하면 수집 시 Context Interview가 실행되고, 엔티티/개념 페이지에 클래스 및 속성 메타데이터가 자동으로 추가됩니다. 그래프 시각화에서도 축(axis) 색상 구분, `relations:` 기반 typed edge, 구조적 허브 강조, 레거시 디밍 토글이 자동으로 활성화됩니다 (위 [/wiki-graph 섹션](#wiki-graph--지식-그래프-시각화) 참고).

### 이미 페이지가 많은 위키에 온톨로지를 처음 도입할 때

`/wiki-ontology-init`은 인터뷰 중 언급된 신규 인스턴스만 시드 페이지로 만들어줄 뿐, 기존에 이미 쌓여 있는 수십~수백 개 페이지를 소급 태깅해주지는 않습니다. 이럴 때는 `/wiki-ontology-apply`를 쓰세요:

1. 전부 정밀 태깅하려 하지 말고, 사용자와 함께 **계층을 나누는 계획**을 세우세요 — 예: 핵심 허브에 대응하는 소수 페이지(Tier A)만 `class:`/`relations:`까지 정밀 태깅하고, 나머지 레거시 페이지(Tier B)는 `status: legacy` + `era:` 정도만 가볍게 부여.
2. 그 계획을 `wiki/ontology-apply-plan.yaml`로 작성합니다 (스키마는 `scripts/ontology_apply.py` 상단 docstring 참고 — `default.set`, `rules[].match.{pages,path_contains,path_glob}`, `rules[].set`, `rules[].relations.add`).
3. `python scripts/ontology_apply.py --plan wiki/ontology-apply-plan.yaml`로 먼저 dry-run 실행 후, 규칙별 매칭 수와 미매칭 페이지 목록을 사용자와 검토합니다.
4. 이상 없으면 `--apply`로 재실행 — 기존 프런트매터의 다른 필드와 본문은 그대로 유지한 채 지정한 필드만 병합되고, `relations:`는 중복 없이 추가되며, `wiki/originals/`는 절대 건드리지 않고, `wiki/log.md`에 자동으로 이력이 남습니다.
5. 마지막으로 `/wiki-ontology-validate`로 위반 0건을 확인하세요.
...
- **`ontology_apply.py`는 기본이 dry-run** — 규칙별 매칭 수와 미매칭 페이지 목록을 먼저 검토한 뒤 `--apply`로 재실행하세요
