...
| `/wiki-ontology-apply [--plan <file>]`                 | Bulk-tag many existing pages from a declarative plan when an ontology is introduced/changed |
...
python scripts/build_graph.py --open       # rebuild graph + open browser (ontology-aware if wiki/ontology.yaml exists)
python scripts/ontology_validate.py --save # ontology checks 7-12, run by /wiki-lint when ontology.yaml exists
python scripts/ontology_apply.py --plan wiki/ontology-apply-plan.yaml --apply  # bulk-tag existing pages
