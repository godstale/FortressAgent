...
# Ontology (only meaningful when wiki/ontology.yaml exists)
python scripts/ontology_validate.py --save            # checks 7-12 → wiki/ontology-validation-report.md
python scripts/ontology_apply.py --plan wiki/ontology-apply-plan.yaml            # dry-run bulk tag
python scripts/ontology_apply.py --plan wiki/ontology-apply-plan.yaml --apply    # write bulk tag
...
scripts/
  build_graph.py       ← outputs graph/graph.json + graph/graph.html (vis.js); ontology-aware when wiki/ontology.yaml exists
  lint.py              ← structural + graph-aware health checks
  ontology_validate.py ← ontology checks 7-12 against wiki/ontology.yaml (invoked by /wiki-lint)
  ontology_apply.py    ← bulk-apply a tagging plan to existing pages (/wiki-ontology-apply)
