...
# Ontology axis → color, per references/ontology-commands.md. Used to color
# nodes by `class:` frontmatter (resolved to its axis) when wiki/ontology.yaml
# declares that axis active — overrides TYPE_COLORS for tagged pages only.
AXIS_COLORS = {
    "Actor": "#1976D2",
    "Activity": "#388E3C",
    "Artifact": "#F57C00",
...
# Generic, project-agnostic convention: pages whose frontmatter `status:` is
# one of these are dimmed in the graph (toggleable) regardless of whether an
# ontology is active — this is a common "active vs. legacy corpus" pattern.
LEGACY_STATUSES = {"legacy", "archived", "deprecated"}
...
def parse_frontmatter(content: str) -> dict:
    """Full YAML frontmatter parse. Returns {} if PyYAML is unavailable or parse fails
    (degrades gracefully to the old regex-only type/title extraction)."""
    if not HAS_YAML:
        return {}
...
def load_ontology() -> dict | None:
    """Load wiki/ontology.yaml. Ontology is opt-in — returns None if absent,
    unparsable, or PyYAML is missing (all graceful degradation, no hard error)."""
    if not HAS_YAML or not ONTOLOGY_FILE.exists():
        return None
    try:
        data = yaml.safe_load(read_file(ONTOLOGY_FILE))
        return data if isinstance(data, dict) else None
    except Exception:
        return None


def build_class_axis_index(ontology: dict) -> dict[str, str]:
    """class_name -> axis_name, for classes under axes marked active: true."""
    index: dict[str, str] = {}
    for axis_name, axis_def in (ontology.get("axes") or {}).items():
        if not axis_def or axis_def.get("active") is False:
            continue
        for cls_name in (axis_def.get("default_classes") or {}) or {}:
            index[cls_name] = axis_name
    return index
...
def build_nodes(pages: list[Path], class_axis: dict[str, str] | None = None):
    """Returns (nodes, relations_by_id) where relations_by_id maps node id ->
    the page's raw `relations:` frontmatter list (consumed by build_typed_edges)."""
    nodes = []
    relations_by_id: dict[str, list[dict]] = {}
    for p in pages:
        content = read_file(p)
        fm = parse_frontmatter(content)
        node_type = fm.get("type") or extract_frontmatter_type(content)
        title_match = re.search(r'^title:\s*"?([^"\n]+)"?', content, re.MULTILINE)
        label = fm.get("title") or (title_match.group(1).strip() if title_match else p.stem)
        body = re.sub(r"^---\n.*?\n---\n?", "", content, flags=re.DOTALL)
        preview_lines = [line.strip() for line in body.splitlines() if line.strip()]
        preview = " ".join(preview_lines[:3])[:220]

        cls = fm.get("class")
        axis = (class_axis or {}).get(cls) if cls else None
        status = fm.get("status")
        legacy = isinstance(status, str) and status.lower() in LEGACY_STATUSES
        color = AXIS_COLORS[axis] if axis in AXIS_COLORS else TYPE_COLORS.get(node_type, TYPE_COLORS["unknown"])

        nid = page_id(p)
        rels = fm.get("relations")
        if isinstance(rels, list) and rels:
            relations_by_id[nid] = [r for r in rels
    ...
    return nodes, relations_by_id


def build_typed_edges(pages: list[Path], relations_by_id: dict[str, list[dict]],
                       ontology: dict) -> list[dict]:
    """Edges from `relations:` frontmatter (predicate/target pairs), per the
    Ontology-Aware Graph Building spec in references/ontology-commands.md.
    Skips drawing a duplicate reverse edge when the predicate declares an
    `inverse` that the target page also uses back toward the source."""
    stem_map = {p.stem.lower(): page_id(p) for p in pages}
    predicate_defs = (ontology.get("relations") or {}) if ontology else {}
    edges = []
    seen = set()
    for src_id, rels in relations_by_id.items():
        for rel in rels:
            pred = rel.get("predicate")
            target_name = str(rel.get("target"))
            target_id = stem_map.get(target_name.lower())
            if not target_id or target_id == src_id:
                continue
            pdef = predicate_defs.get(pred) or {}
            inverse = pdef.get("inverse")
            if inverse and (target_id, src_id, inverse) in seen:
                continue
            key = (src_id, target_id, pred)
            if key in seen:
                continue
            seen.add(key)
            edges.append({
                "id": f"{src_id}->{target_id}:TYPED:{pred}",
                "from": src_id,
                "to": target_id,
                "type": "TYPED",
                "predicate": pred,
                "label": pred,
                "color": EDGE_COLORS["TYPED"],
                "dashes": True,
                "confidence": 1.0,
            })
    return edges
...
def deduplicate_edges(edges: list[dict]) -> list[dict]:
    ...
    Keyed on (pair, type, predicate) — not just the pair — so a TYPED relation
    edge and an EXTRACTED wikilink edge between the same two pages coexist
    instead of one silently dropping the other."""
    best = {}  # (min(a,b), max(a,b), type, predicate) -> edge
    for e in edges:
        a, b = e["from"], e["to"]
        key = (min(a, b), max(a, b), e.get("type", "INFERRED"), e.get("predicate", ""))
...
def render_html(nodes: list[dict], edges: list[dict],
                 ontology_active: bool = False, active_axes: list[str] | None = None) -> str:
    ...
    if ontology_active and active_axes:
        legend_items = "".join(
            f'<span style="background:{AXIS_COLORS.get(a, "#888")};padding:3px 8px;margin:2px;border-radius:3px;font-size:12px">{a}</span>'
    ...
    n_typed = len([e for e in edges if e.get('type') == 'TYPED'])
    n_legacy = len([n for n in nodes if n.get('legacy')])
...
<div id="controls">
  ...
  <div class="filter-group">
    ...
    <div class="cb-row"><input type="checkbox" id="cb-typed" checked onchange="applyFilters()"><span style="color:#00BCD4">┅</span> Typed / relations ({n_typed})</div>
  </div>
  <div class="filter-group">
  ...
  <div class="filter-group">
    <label>Status</label>
    <div class="cb-row"><input type="checkbox" id="cb-dim-legacy" checked onchange="applyFilters()">Dim legacy pages ({n_legacy})</div>
  </div>
  <p>Click a node to highlight its connected neighbors and view the markdown on the right. Click the background to restore the full graph. Star-shaped nodes are structural hubs (degree &gt; mean + 2σ).</p>
</div>
<div id="graph"></div>
<aside id="drawer">
...
const controls = {{
  ...
  typed: document.getElementById("cb-typed"),
  dimLegacy: document.getElementById("cb-dim-legacy"),
...
function currentEdgeState() {{
  ...
  return {{
    ...
    showTyped: controls.typed.checked,
    minConf,
  }};
}}
...
function passesEdgeFilters(edge, edgeState) {{
  const typeOk = (edge.type === "EXTRACTED" && edgeState.showExtracted)
    ...
    || (edge.type === "TYPED" && edgeState.showTyped);
...
function openDrawer(node, relatedIds) {{
  ...
  const classText = node.class ? ` · class:${{node.class}}` : "";
  const statusText = node.status ? ` · status:${{node.status}}` : "";
  document.getElementById("drawer-meta").textContent = `${{node.type}}${{classText}}${{statusText}}${{communityText}}`;
...
function applyFilters(query = searchInput.value, selectedNodeId = activeNodeId) {{
  ...
  const nodeUpdates = originalNodes.map(node => {{
    ...
    const dimmedLegacy = controls.dimLegacy.checked && !!node.legacy && !isActive;
    const emphasized = matchesSearch && isRelated && (isConnected || !!lower || isActive) && !dimmedLegacy;
    ...
    return {{
      id: node.id,
      hidden,
      shape: node.hub ? "star" : "dot",
      color: {{
        background: emphasized ? node.color : hexToRgba(node.color, hidden ? 0.05 : (dimmedLegacy ? 0.10 : 0.14)),
        border: emphasized ? hexToRgba(node.color, 0.96) : hexToRgba(node.color, hidden ? 0.08 : (dimmedLegacy ? 0.14 : 0.22)),
      ...
      size: isActive ? 18 : (node.hub ? 16 : 12),
    }};
  }});
...
def build_graph(open_browser: bool = False, report: bool = False, save: bool = False):
    ...
    # Ontology-aware mode: opt-in, activates only when wiki/ontology.yaml exists
    # and PyYAML is installed. See references/ontology-commands.md.
    ontology = load_ontology()
    class_axis = build_class_axis_index(ontology) if ontology else {}
    active_axes = sorted(set(class_axis.values()))
    ontology_active = bool(class_axis)
    if ontology_active:
        print(f"  Ontology-aware mode: axes={active_axes}")

    # Extract wikilink edges
    print("  Extracting wikilinks...")
    nodes, relations_by_id = build_nodes(pages, class_axis)
    edges = build_extracted_edges(pages)
    print(f"  → {len(edges)} extracted edges")

    if ontology_active:
        typed_edges = build_typed_edges(pages, relations_by_id, ontology)
        print(f"  → {len(typed_edges)} typed relation edges")
    ...
    # Community detection. Node color is community-based normally, but when an
    # ontology is active, axis colors (already set in build_nodes) take
    # precedence — `group` still carries the numeric community id so
    # lint.py's graph-aware checks (which key off it) keep working either way.
    print("  Running Louvain community detection...")
    communities = detect_communities(nodes, edges)
    for node in nodes:
        comm_id = communities.get(node["id"], -1)
        if not ontology_active and comm_id >= 0:
            node["color"] = COMMUNITY_COLORS[comm_id % len(COMMUNITY_COLORS)]
        node["group"] = comm_id

    # Compute degree-based node sizing (value) and structural hub detection
    # (degree > mean + 2*std, same threshold as lint.py's hub-stub check).
    degree_map: dict[str, int] = {}
    for e in edges:
        degree_map[e["from"]] = degree_map.get(e["from"], 0) + 1
        degree_map[e["to"]] = degree_map.get(e["to"], 0) + 1
    deg_values = list(degree_map.values())
    hub_threshold = None
    if len(deg_values) >= 2:
        hub_threshold = statistics.mean(deg_values) + 2 * statistics.stdev(deg_values)
    for node in nodes:
        deg = degree_map.get(node["id"], 0)
        node["value"] = deg + 1  # +1 so isolated nodes are still visible
        node["hub"] = bool(hub_threshold is not None and deg > hub_threshold)

    # Save graph.json
    graph_data = {"nodes": nodes, "edges": edges, "built": today, "ontology_active": ontology_active}
    ...
    # Save graph.html
    html = render_html(nodes, edges, ontology_active=ontology_active, active_axes=active_axes)
    GRAPH_HTML.write_text(html, encoding="utf-8")
    print(f"  saved: graph/graph.html")

    n_typed_final = len([e for e in edges if e.get("type") == "TYPED"])
    append_log(f"## [{today}] graph | Knowledge graph rebuilt\n\n"
               f"{len(nodes)} nodes, {len(edges)} edges "
               f"({len(edges) - n_typed_final} extracted, {n_typed_final} typed)."
               + (f" Ontology-aware: axes={active_axes}." if ontology_active else ""))
