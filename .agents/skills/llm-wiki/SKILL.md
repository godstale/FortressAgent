...
| **Ontology** | `/wiki-ontology-*` (incl. `/wiki-ontology-apply` for bulk retro-tagging) | `references/ontology-commands.md` |
...
2. **Strategy:** Select mode (Full vs. Summary-Only).
   - _Summary-Only:_ `.xlsx`, `.pptx`, `.pdf` > 30p, or `--summary-only` flag.
3. **Execution:**
...
- **/wiki-lint**: Run `python scripts/lint.py --save` for orphans, broken links, stale summaries, graph-aware checks (Report: `wiki/lint-report.md`).
  - **If `wiki/ontology.yaml` exists**, also run `python scripts/ontology_validate.py` — it deterministically implements ontology checks 7–12 (see `references/ontology-commands.md`). Append its `## Ontology Violations` section to `wiki/lint-report.md`. Do **not** re-derive these checks by hand-reading every page; only fall back to manual checking if Python/PyYAML is unavailable.
  - If `wiki-hub.md` exists, ignore/lower severity of broken links originating from imported external files.
- **/wiki-graph**: Run `python scripts/build_graph.py --open` to rebuild `graph/graph.json` and `graph/graph.html`. When `wiki/ontology.yaml` exists, the script automatically switches to ontology-aware styling (axis colors, typed relation edges, legacy dimming) — no manual graph customization needed; see `references/ontology-commands.md`.
...
- **/wiki-ontology-apply**: Bulk-apply a tagging plan (`class`/`status`/`era`/`relations`) to many existing pages at once when an ontology is introduced or changed on a wiki that already has content. See `references/ontology-commands.md`.
...
- **CONSISTENCY:** `wiki/index.md` must be updated on _every_ ingest to keep query discovery working.
