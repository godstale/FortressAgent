...
Announce before proceeding: _"Using summary-only strategy for `<filename>` (reason: <reason>)."_
...
3. **Inline schema validation — act immediately, do not defer:**
   ...
   - New target names → offer: _"`<name>` is new. Create stub page? (yes/no)"_ → on yes, create minimal page with "TODO" body
